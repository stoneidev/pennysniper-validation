...
└────────┬─────────────────────────┘
         ↓
┌──────────────────────────────────┐
│ 2. monthly_retrain.py            │  매월 1일 실행
│    (if 1st of every month)       │  → config/current_rule.json
...
| `monthly_retrain.py` | Re-optimize rule monthly using prior 3 months data |
...
# 2. Train monthly rule (or override with --as-of for backtest)
python scripts/live/monthly_retrain.py --as-of 2025-05-01 --out config/rule_2025_05.json
...
- **monthly_retrain**: ~30 sec (runs only on 1st of each month)
- **daily_report**: ~5 sec
